...
#include <boost/thread/thread.hpp> // Add this line
...
int main(int argc, char **argv) {
    ...
    // STEP1 读取对应的pcd文件 cloud1.pcd cloud2.pcd
    ...
    // STEP2 voxel_filter 对输入的点云进行过滤
    // 只对 input_cloud 进行了滤波处理，减少其数据量到10%左右，而target_cloud 不做滤波处理:
    ...
    // STEP3 初始化NDT并且设置NDT参数，然后进行配准
    pcl::NormalDistributionsTransform<pcl::PointXYZ, pcl::PointXYZ> ndt;
    ndt.setTransformationEpsilon(0.01); // 判断我们的优化过程是否已经收敛到最终解的阈值
    ndt.setStepSize(0.1);               // 设置牛顿法优化的最大步长
    ndt.setResolution(1.0);             // 设置网格化时立方体的边长，网格大小设置在NDT中非常重要，
                                        //太大会导致精度不高，太小导致内存过高，并且只有两幅点云相差不大的情况才能匹配

    ndt.setMaximumIterations(35); // 当迭代次数达到35次或者收敛到阈值时，停止优化过程
    ...
    //STEP5 优化完成后，将优化后的点云保存到文件中，并且进行可视化
    ...
    // STEP6 目标点云（即我们已有的高精度地图）用红点绘制，而输入点云用绿点绘制
